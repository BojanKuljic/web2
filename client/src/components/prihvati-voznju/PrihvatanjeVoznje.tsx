...
import { PrihvatanjeVoznjeProps } from "../../models/props/PrihvatanjeVoznjeProps";
import { DostupnaVoznjaRedTabela } from "./DostupnaVoznjaRedTabela";

export const PrihvatanjeVoznje: React.FC<PrihvatanjeVoznjeProps> = ({
  ProcitajDostupneVoznje,
  PrihvatiVoznju,
}) => {
  const auth = useAuth();
  const [voznje, setVoznje] = useState<PodaciVoznja[]>([]);
  const [currentPage, setCurrentPage] = useState(1);
  const resultsPerPage = 10;

  useEffect(() => {
    const dobaviPodatke = async () => {
      const response: PodaciVoznja[] = await ProcitajDostupneVoznje(
        auth?.token ?? ""
      );

      // if (response.length >= 1 && response.length !== voznje.length) {
      //   toast.success(`Dostupno je ${response.length} novih vožnji.`);
      // }
      setVoznje(response);
    };

    dobaviPodatke(); // Fetch data immediately on mount

    const intervalId = setInterval(dobaviPodatke, 10000); // Fetch data every 10 seconds

    return () => clearInterval(intervalId); // Cleanup interval on component unmount
  }, [ProcitajDostupneVoznje, auth?.token]); // Re-run effect if auth.token changes

  const totalPages = Math.ceil(voznje.length / resultsPerPage);
  ...
  const paginatedVoznje = voznje.slice(
          ...
          {voznje.length === 0 ? (
            <>
              <div className="mx-auto max-w-screen-2xl px-4 md:px-8">
                <div className="flex flex-col overflow-hidden rounded-lg shadow-xl bg-white/90 sm:flex-row md:h-80">
                  <div className="flex w-full flex-col p-4 sm:w-1/2 sm:p-8 lg:w-2/5">
                    ...
                    <p className="mb-8 max-w-md text-gray-800">
                      U našem sistemu nije registrovana nijedna dostupna vožnja.
                      To nužno ne mora da predstavlja problem, ali za svaki
                      slučaj osvežite stranicu.
                    </p>
                    <div className="mt-auto">
                      <a
                        href="/dostupne-voznje"
                    ...
                    <div className="overflow-hidden border border-gray-950 rounded-xl shadow-lg">
                      <table className="min-w-full divide-y divide-gray-700 text-center">
                        ...
                        <tbody className="divide-gray-700 bg-gray-900/60">
                          {paginatedVoznje.map(
                            (voznja: PodaciVoznja, index: number) => (
                              <DostupnaVoznjaRedTabela
                                key={index}
                                voznja={voznja}
