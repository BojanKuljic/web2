...
import { PrethodneVoznjeProps } from "../../models/props/PrethodneVoznjeProps";
import { PodaciVoznja } from "../../models/rides/PodaciVoznja";
import { TabelaVoznja } from "./VoznjeTabela";

export const PrethodneVoznje: React.FC<PrethodneVoznjeProps> = ({
  ProcitajVoznje,
}) => {
  const auth = useAuth();
  ...
  useEffect(() => {
    const dobaviPodatke = async () => {
      const response: PodaciVoznja[] = await ProcitajVoznje(
        auth?.token ?? "",
        parseInt(auth?.id ?? "0")
      );
      setVoznje(response);
    ...
    dobaviPodatke();
  }, [ProcitajVoznje, auth?.id, auth?.token]);
                ...
                <div className="flex flex-col overflow-hidden rounded-lg shadow-xl bg-white/90 sm:flex-row md:h-80">
                  <div className="flex w-full flex-col p-4 sm:w-1/2 sm:p-8 lg:w-2/5">
                    ...
                    <p className="mb-8 max-w-md text-gray-800">
                      U našem sistemu nije registrovana nijedna vožnja. To nužno
                        ...
                        <tbody className="divide-gray-700 bg-gray-900/60">
                          {paginatedVoznje.map(
                            (voznja: PodaciVoznja, index: number) => (
                              <TabelaVoznja key={index} voznja={voznja} />
                            )
                          )}
                        </tbody>
                      </table>
